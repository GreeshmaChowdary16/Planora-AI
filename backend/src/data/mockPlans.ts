...
export const mockPlans: ProjectPlan[] = [
  {
    projectName: 'AI-Powered Personalized Diet Planner',
    ...
    roadmap: [
      {
        ...
        tasks: [
          { id: 'diet-1', title: 'Initialize Project Repository', description: 'Configure React frontend with Tailwind CSS and set up Node/Express folder structures.', duration: '3 days', tools: ['Vite', 'React.js', 'Tailwind CSS', 'npm'] },
          { id: 'diet-2', title: 'Build User Profile Form', description: 'Create multi-step input screens capturing age, weight, height, allergens, and dietary goals.', duration: '4 days', tools: ['React Hook Form', 'Zod', 'Lucide React'] }
        ]
      },
      {
        phaseName: 'Phase 2: Backend & Gemini API Integration',
        tasks: [
          { id: 'diet-3', title: 'Set up Express API Server', description: 'Configure basic Express routing and set up environment variables for API safety.', duration: '3 days', tools: ['Node.js', 'Express', 'TypeScript', 'dotenv'] },
          { id: 'diet-4', title: 'Integrate Gemini API Client', description: 'Create utility functions to prompt Gemini 2.5 Flash and return structured JSON recipes and macro targets.', duration: '5 days', tools: ['groq-sdk', 'Node.js', 'JSON.parse'] }
        ]
      },
      {
        phaseName: 'Phase 3: Core Features & USDA Lookup',
        tasks: [
          { id: 'diet-5', title: 'USDA Food Central Integration', description: 'Connect third-party nutritional database to allow search of raw ingredients and caloric values.', duration: '4 days', tools: ['Axios', 'USDA API Key'] },
          { id: 'diet-6', title: 'Implement Dashboard Visualizer', description: 'Build interactive progress bar widgets showing protein, carb, fat, and hydration percentages.', duration: '5 days', tools: ['Recharts', 'Tailwind CSS'] }
        ]
      },
      {
        phaseName: 'Phase 4: Optimization & Deployment',
        tasks: [
          { id: 'diet-7', title: 'Mobile UX Refinement', description: 'Ensure the macro logs and recipes are fully responsive and feel native on mobile screens.', duration: '4 days', tools: ['Chrome DevTools', 'Tailwind CSS'] },
          { id: 'diet-8', title: 'Deploy Web Apps', description: 'Host the React client on Vercel and backend server on Render, connecting environment variables.', duration: '4 days', tools: ['Vercel CLI', 'Render CLI', 'Git'] }
        ]
      }
    ],
    architecture: {
  ...
  {
    projectName: 'Smart Plant Irrigation IoT System',
    ...
    roadmap: [
      {
        ...
        tasks: [
          { id: 'iot-1', title: 'Hardware Wiring Setup', description: 'Wire ESP32, capacitive moisture sensor, relay switch, and water pump on breadboard.', duration: '4 days', tools: ['ESP32 Board', 'Moisture Sensor', '5V Relay Switch', 'Jumper Wires'] },
          { id: 'iot-2', title: 'Write Basic ESP32 Firmware', description: 'Write basic C++ code to read moisture sensor values and print to serial logs.', duration: '5 days', tools: ['Arduino IDE', 'C++', 'ESP32 Sketch Core'] }
        ]
      },
      {
        phaseName: 'Phase 2: Cloud Communication Setup',
        tasks: [
          { id: 'iot-3', title: 'Set up HiveMQ Broker', description: 'Create a free HiveMQ cloud broker instance and verify client publishing capabilities.', duration: '4 days', tools: ['HiveMQ Console', 'MQTT Box Client'] },
          { id: 'iot-4', title: 'Write MQTT ESP32 client', description: 'Code the ESP32 to connect to local WiFi and publish sensor data to the broker every 15 minutes.', duration: '6 days', tools: ['PubSubClient', 'WiFi Library', 'Arduino IDE'] }
        ]
      },
      {
        phaseName: 'Phase 3: Database & Cloud Integration',
        tasks: [
          { id: 'iot-5', title: 'MQTT to Firebase Bridge', description: 'Write a small Node.js server bridge or utilize cloud functions to forward MQTT payloads into Firebase DB.', duration: '5 days', tools: ['Node.js', 'Firebase SDK', 'mqtt-node-client'] },
          { id: 'iot-6', title: 'Configure Firebase Security Rules', description: 'Secure the database endpoints while keeping read operations open for the web dashboard.', duration: '3 days', tools: ['Firebase Console', 'JSON Config Rules'] }
        ]
      },
      {
        phaseName: 'Phase 4: Web Dashboard Development',
        tasks: [
          { id: 'iot-7', title: 'Build React charts', description: 'Integrate custom chart components to visualize moisture levels over the past 24 hours.', duration: '6 days', tools: ['React.js', 'ChartJS', 'Tailwind CSS'] },
          { id: 'iot-8', title: 'Implement Manual Watering Trigger', description: 'Build a button in React that writes to MQTT and triggers the ESP32 relay to open the water pump.', duration: '5 days', tools: ['mqtt.js client', 'React State Hook'] }
        ]
      }
    ],
    architecture: {
  ...
  {
    projectName: 'CollabCode – Real-time Peer IDE',
    ...
    roadmap: [
      {
        ...
        tasks: [
          { id: 'code-1', title: 'Integrate Monaco Editor', description: 'Mount Monaco Editor inside React and set up custom syntax themes.', duration: '5 days', tools: ['React.js', '@monaco-editor/react'] },
          { id: 'code-2', title: 'Set up Express Socket Server', description: 'Build basic Node server that opens Socket.io listeners and handles room joining/leaving.', duration: '5 days', tools: ['Node.js', 'Express', 'Socket.io'] }
        ]
      },
      {
        phaseName: 'Phase 2: Collaborative Synchronization',
        tasks: [
          { id: 'code-3', title: 'Implement Yjs Sync', description: 'Connect Monaco text models with Yjs document stores and link them via Websocket providers.', duration: '7 days', tools: ['Yjs CRDT', 'y-websocket', 'y-protocols'] },
          { id: 'code-4', title: 'Build Telemetry Indicators', description: 'Implement floating user cursors and highlighting indicating which developer is typing.', duration: '6 days', tools: ['React Hooks', 'Tailwind CSS'] }
        ]
      },
      {
        phaseName: 'Phase 3: Sandbox Code Execution',
        tasks: [
          { id: 'code-5', title: 'Docker Runner Setup', description: 'Configure Docker containers that read a string of code, execute it securely, and capture stdout.', duration: '8 days', tools: ['Docker Engine', 'Bash Scripting', 'Security sandbox'] },
          { id: 'code-6', title: 'Create Sandbox API endpoints', description: 'Build a secure POST endpoint on Express to run Python, JS, and C++ files, integrating timeout limits.', duration: '7 days', tools: ['Node.js', 'Dockerode SDK', 'Express API'] }
        ]
      },
      {
        phaseName: 'Phase 4: File Tree & UI Polish',
        tasks: [
          { id: 'code-7', title: 'Construct Virtual Directory', description: 'Build a sidebar file explorer allowing creation, deletion, and renaming of files in the editor.', duration: '6 days', tools: ['React-Arborist', 'Lucide React'] },
          { id: 'code-8', title: 'Deploy Sandbox Server', description: 'Deploy the main app to digital ocean or AWS EC2, configuring Docker socket permissions.', duration: '6 days', tools: ['AWS EC2', 'Nginx', 'Docker Compose'] }
        ]
      }
    ],
    architecture: {
...
export const fallbackPlan: ProjectPlan = {
  projectName: 'Custom AI Project Plan',
  ...
  roadmap: [
    {
      ...
      tasks: [
        { id: 'custom-1', title: 'Initialize Workspace', description: 'Setup code repository and clean project scaffolding.', duration: '2 days', tools: ['Vite', 'React.js', 'Git'] },
        { id: 'custom-2', title: 'Design Core Interface', description: 'Wireframe user input flows and setup styling themes.', duration: '3 days', tools: ['Tailwind CSS', 'Framer Motion'] }
      ]
    },
    {
      phaseName: 'Phase 2: Feature Development',
      tasks: [
        { id: 'custom-3', title: 'Implement Primary Logic', description: 'Build component behaviors and state machines.', duration: '5 days', tools: ['React Hooks', 'Context API'] }
      ]
    }
  ],
  architecture: {
...
mockPlans.forEach(plan => {
  const id = (plan.projectName || '').toLowerCase();
